...
class CalculatorViewController: UIViewController {
    ...
    //if the negative button is pressed, a variable is created if there is a Double casted text from the output label that is NOT blank. if so, then the variable negativeNumber will be calculated. We do this with simple math. We take the Double casted string called negative number and multiply it by -1. Which is how you get the negative value of an integer.
    @IBAction func negPosButtonPressed(_ sender: Any) {
        if var negativeNumber = Double(outputLabel.text ?? "") {
            negativeNumber = (negativeNumber * -1)
            outputLabel.text = String(negativeNumber)
            brain.operand1String = String(negativeNumber)
        }
    }
